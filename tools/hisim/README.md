...
> **Notes**:
> - Use `--sim-config-path` to specify the simulation configuration file, which is equivalent to the system environment variable `HISIM_CONFIG_PATH`.
...
Hisim uses **dynamic interception** to hijack the execution flow of the inference framework, bypassing actual LLM computation.
